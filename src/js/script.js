document.addEventListener("DOMContentLoaded", () => {
    ...
    if (venues[selectedEvent]) {
      venues[selectedEvent]
        .filter(venue => {
          ...
          const meetsFacility = !selectedFacility || venue.facilities.includes(selectedFacility);
  ...
  eventForm.addEventListener("submit", (e) => {
    ...
    const pricing = {
      catering: {
        indian: 500, continental: 700, chinese: 600,
        italian: 800, mexican: 900, japanese: 1000,
        thai: 750, vegan_special: 850
      },
    ...
    const subtotal = Object.values(costs).reduce((total, cost) => total + cost, 0);
    ...
    const grandTotal = subtotal + tax + serviceCharge + convenienceFee + securityDeposit;
    ...
    summaryContent.innerHTML = `
      ...
      <div class="space-y-2">
        ...
        <div class="mt-4">
          ...
          <p>Catering (${formData.catering}): ₹${costs.catering.toLocaleString()}</p>
          <p>Decoration (${formData.decoration}): ₹${costs.decoration.toLocaleString()}</p>
          <p>Entertainment (${formData.entertainment}): ₹${costs.entertainment.toLocaleString()}</p>
